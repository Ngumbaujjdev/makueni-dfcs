...
if (isset($_POST['displayRecentAgrovetTransactions'])):
    $app = new App;
    ...
    $account_query = "SELECT id FROM agrovet_accounts WHERE agrovet_id = $agrovet_id";
    $account_result = $app->select_one($account_query);
    $agrovet_account_id = $account_result->id ?? 0;
    
    $query = "SELECT 
                aat.id,
                aat.transaction_type,
                aat.amount,
                aat.reference_id,
                aat.description,
                aat.processed_by,
              ...
              LEFT JOIN users u ON aat.processed_by = u.id
              WHERE aat.agrovet_account_id = $agrovet_account_id
...
<div class="card custom-card">
    <div class="card-header justify-content-between">
        <div class="card-title">
            <i class="ri-exchange-dollar-line me-2"></i> Recent Agrovet Account Transactions
        </div>
        <div>
            <a href="agrovet-transactions" class="btn btn-sm btn-outline-primary">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="datatable-recent-agrovet-transactions" class="table table-bordered text-nowrap w-100">
                <thead>
                    <tr>
...
$(document).ready(function() {
    $('#datatable-recent-agrovet-transactions').DataTable({
        responsive: true,
        order: [
            [0, 'desc']
