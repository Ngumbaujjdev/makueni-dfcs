...
if (isset($_POST['id'])) {
    try {
        ...
        if ($staff->staff_id !== $_POST['staff_id']) {
          $staffIdQuery = "SELECT COUNT(*) as count FROM sacco_staff 
                WHERE staff_id = :new_staff_id AND id != :current_id";
                 $staffIdResult = $app->selectOne($staffIdQuery, [
        ...
        // Update sacco_staff table
        $staffQuery = "UPDATE sacco_staff SET 
                      position = :position,
                      staff_id = :staff_id,
        ...
        $activityParams = [
            ...
            ':activity_type' => 'sacco_staff_updated',
            ':description' => "SACCO staff updated: {$_POST['first_name']} {$_POST['last_name']}"
        ];
