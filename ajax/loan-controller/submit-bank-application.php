...
// Ensure output buffering is on to capture any accidental output
ob_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $provider_type = 'bank'; // Using bank as the provider
        $bank_id = 1; // Hard-coded to Kenya Commercial Bank (ID: 1)
        ...
        $loanTypeQuery = "SELECT lt.*, b.name as bank_name 
                         FROM loan_types lt
                         JOIN banks b ON lt.bank_id = b.id
                         WHERE lt.id = :loan_type_id";
        ...
        // Creditworthiness assessment functions
        ...
        // Get financial snapshot data for bank loan applications
        function getBankFinancialSnapshot($app, $farmer_id) {
            ...
            // Calculate credit capacity for bank loans (more conservative than SACCO)
            $creditCapacity = calculateBankCreditCapacity($deliveries->value, $activeLoans->balance, $inputCredits->balance);
        ...
        // Calculate credit capacity specifically for bank loans (more conservative approach)
        function calculateBankCreditCapacity($deliveriesValue, $activeLoansBalance, $inputCreditsBalance) {
            ...
            // Banks typically use more conservative debt-to-income ratios
            $debtToIncomeRatio = $totalDebt / ($monthlyIncome * 6);
            
            // Bank-specific credit capacity calculation (more stringent than SACCO)
            if ($debtToIncomeRatio <= 0.15) return 95; // Excellent: debt is less than 15% of 6-month income
            if ($debtToIncomeRatio <= 0.25) return 85; // Very Good: debt is 15-25% of 6-month income
            if ($debtToIncomeRatio <= 0.35) return 70; // Good: debt is 25-35% of 6-month income
            if ($debtToIncomeRatio <= 0.50) return 50; // Fair: debt is 35-50% of 6-month income
            if ($debtToIncomeRatio <= 0.65) return 30; // Poor: debt is 50-65% of 6-month income
            return 15; // Very poor: debt exceeds 65% of 6-month income
        }
        
        // Get bank-specific information for the response
        function getBankInformation($app, $loan_type_id) {
            $bankInfoQuery = "SELECT lt.name as loan_type_name, lt.interest_rate, 
                                     b.name as bank_name, b.id as bank_id
                              FROM loan_types lt
                              JOIN banks b ON lt.bank_id = b.id
                              WHERE lt.id = :loan_type_id";
            
            $bankInfo = $app->selectOne($bankInfoQuery, [':loan_type_id' => $loan_type_id]);
            
            return [
                'bank_name' => $bankInfo->bank_name ?? 'Bank Partner',
                'loan_type_name' => $bankInfo->loan_type_name ?? 'Bank Loan Product',
                'interest_rate' => $bankInfo->interest_rate ?? 0,
                'bank_id' => $bankInfo->bank_id ?? null
            ];
        }


        // Insert loan application with bank_id
        ...
        $logParams = [
            ...
            ':description' => "Bank loan application submitted. Amount: KES " . 
        ...
        $activityParams = [
            ':user_id' => $_SESSION['user_id'],
            ':description' => "Bank loan application submitted - Ref: " . $reference_number . 
        ...
        // Get bank financial snapshot data
        $bankFinancialSnapshot = getBankFinancialSnapshot($app, $farmer_id);
        
        // Get bank information for response
        $bankInformation = getBankInformation($app, $loan_type_id);

        // Commit transaction
        $app->commit();
        ob_end_clean();

       // Return enhanced response with bank-specific data
       echo json_encode([
        'success' => true,
        ...
        'financial_snapshot' => $bankFinancialSnapshot,
        'bank_processing' => [
            'expected_review_time' => '3-5 business days',
            'bank_contact_info' => 'The bank may contact you directly for additional documentation',
            'next_steps' => [
                'Bank will review your creditworthiness assessment',
                'Additional documentation may be requested',
                'Bank will conduct their own verification process',
                'Final decision will be communicated within 5-7 business days'
            ]
        ],
        'recommendations' => [
            'maintain_deliveries' => 'Continue consistent produce deliveries during review period',
            'documentation_ready' => 'Keep income documentation ready if requested by bank',
            'contact_availability' => 'Ensure you are available for bank contact during business hours'
        ]
    ]);
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Error submitting bank loan application: ' . $e->getMessage()
        ]);
    }
} else {
    echo json_encode([
