...
<body>
        ...
        <div class="main-content app-content">
            <div class="container-fluid">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between my-2 page-header-breadcrumb">
                    <h1 class="page-title fw-semibold fs-18 mb-0">Add Agrovet Staff</h1>
                            ...
                            <div class="card-body">
                                <div class="row gy-3">
                                    <div class="col-xl-12">
                                        ...
                                        <select class="form-control" id="agrovet-id" required>
                                            <option value="">Select an agrovet</option>
                                            <?php
                                                 $app = new App;
                                                 $query = "SELECT * FROM agrovets ORDER BY name ASC";
                                                 $agrovets = $app->select_all($query);
                                                 if($agrovets):
                                                     foreach($agrovets as $agrovet): ?>
                                            <option value="<?php echo $agrovet->id ?>"><?php echo $agrovet->name ?> -
                                                <?php echo $agrovet->location ?></option>
                                            <?php endforeach;
                                                 endif; ?>
                                        </select>
                                    </div>
                                ...
                                <div class="d-flex justify-content-end mt-4">
                                    <button class="btn text-white" onclick="submitAgrovetStaff()"
    ...
    <script>
    function submitAgrovetStaff() {
        // Get form values
        const formData = new FormData();
        formData.append('agrovet_id', $("#agrovet-id").val());
        ...
        if (!$("#agrovet-id").val() || !$("#first-name").val() || !$("#last-name").val() ||
            !$("#position").val() || !$("#employee-number").val() || !$("#phone").val() ||
            !$("#email").val() || !$("#password").val() || !$("#confirm-password").val()) {
        ...
        if ($("#password").val().length < 4) {
            toastr.error('Password must be at least 4 characters long', 'Error');
            return;
        }
        ...
        $.ajax({
            url: 'http://localhost/dfcs/ajax/agrovet-controller/add-staff.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
