...
<body>
        ...
        <div class="main-content app-content">
            <div class="container-fluid">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between my-2 page-header-breadcrumb">
                    <h1 class="page-title fw-semibold fs-18 mb-0">Add Bank Staff</h1>
                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            <div class="card-header justify-content-between">
                                <div class="card-title">Add New Bank Staff Member</div>
                            </div>
                            <div class="card-body">
                                <div class="row gy-3">
                                    <div class="col-xl-12">
                                        <label class="form-label">Select Bank</label>
                                        <select class="form-control" id="bank-id" required>
                                            ...
                                            <option value="<?php echo $bank->id ?>"><?php echo $bank->name ?> -
                                                <?php echo $bank->branch ?></option>
                                            <?php endforeach;
                                                endif; ?>
                                        </select>
                                    </div>
                                ...
                                <div class="d-flex justify-content-end mt-4">
                                    <button class="btn text-white" onclick="submitBankStaff()"
    ...
    <script>
    function submitBankStaff() {
        // Get form values
        const formData = new FormData();
        formData.append('bank_id', $("#bank-id").val());
        formData.append('first_name', $("#first-name").val());
        formData.append('last_name', $("#last-name").val());
        formData.append('email', $("#email").val());
        formData.append('password', $("#password").val());
        formData.append('position', $("#position").val());
        formData.append('staff_id', $("#staff-id").val());
        formData.append('department', $("#department").val());

        // Validate required fields
        if (!$("#bank-id").val() || !$("#first-name").val() || !$("#last-name").val() ||
            !$("#email").val() || !$("#password").val() || !$("#position").val() ||
            !$("#staff-id").val() || !$("#department").val()) {
            toastr.error('Please fill in all required fields', 'Error');
            return;
        }
        //   ajax request to submit the form
        $.ajax({
            url: 'http://localhost/dfcs/ajax/bank-controller/add-staff.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
                    ...
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.location.href =
                                'http://localhost/dfcs/admin/bank-staff/view-data';
                        }, 2000);
                    } else {
