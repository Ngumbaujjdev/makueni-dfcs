...
<body>
            ...
            <div class="container-fluid">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                    <div>
                        ...
                        <?php endif; ?>
                        <span class="fs-semibold text-muted pt-5">Bank Management Dashboard Staff</span>
                    </div>
                </div>
                <!-- End::page-header -->
                <div class="row mt-2">
                                ...
                                <div class="d-flex align-items-top justify-content-between">
                                    <div>
                                        <span class="avatar avatar-md avatar-rounded"
                                            ...
                                            <i style="color:#6AA32D" class="fa-solid fa-users fs-16"></i>
                                        </span>
                                    </div>
                                    <div class="flex-fill ms-3">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div>
                                                <p class="text-white mb-0">Total Bank Staff</p>
                                                <?php
                                                     $query = "SELECT COUNT(*) as count FROM bank_staff";
                                                     $result = $app->select_one($query);
                                                     $staff_count = ($result) ? $result->count : 0;
                                                 ?>
                            ...
                            <div class="card-body">
                                <div class="d-flex align-items-top justify-content-between">
                                    ...
                                    <div class="flex-fill ms-3">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div>
                                                <p class="text-muted mb-0" style="color:#6AA32D;">Active Bank Staff</p>
                                                <?php
                                                       $query = "SELECT COUNT(*) as count FROM bank_staff WHERE is_active = 1";
                                                       $result = $app->select_one($query);
                                                       $active_staff = ($result) ? $result->count : 0;
                                                   ?>
    ...
    function displayBankStaff() {
        let displayBankStaff = "true";
        $.ajax({
            url: "http://localhost/dfcs/ajax/bank-controller/display-staff.php",
            type: 'POST',
            data: {
                displayBankStaff: displayBankStaff,
            },
            success: function(data, status) {
                $('#displayBankStaff').html(data);
            },
        });
    }

    function deleteBankStaff(deleteId) {
        if (confirm("Are you sure you want to delete this bank staff member?")) {
            $.ajax({
                url: "http://localhost/dfcs/ajax/bank-controller/delete-staff.php",
                type: "POST",
                data: {
                    deletesend: deleteId
                },
                success: function(data, status) {
                    toastr.success('Bank staff member deleted successfully', 'Success', {
    ...
    function editBankStaff(staffId) {
        window.location.href = 'http://localhost/dfcs/admin/bank-staff/update-data?id=' + staffId;
    }
    </script>
</body>
