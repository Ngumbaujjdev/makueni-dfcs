...
<body>
            ...
            <div class="container-fluid">
                <?php 
                   if (isset($_GET['id'])) {
                       ...
                       $bank = $app->selectOne($query, [':id' => $id]);
                   ?>
                <!-- Page Header -->
                <div class="d-md-flex d-block align-items-center justify-content-between my-2 page-header-breadcrumb">
                    <h1 class="page-title fw-semibold fs-18 mb-0">Update Bank</h1>
                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            <div class="card-header justify-content-between">
                                <div class="card-title">Update Bank Details</div>
                            </div>
                            <div class="card-body">
                                <input type="hidden" id="bank-id" value="<?php echo $bank->id ?>">

                                <div class="row gy-3">
                                    <div class="col-xl-6">
                                        <label class="form-label">Bank Name</label>
                                        <input type="text" class="form-control" id="bank-name"
                                            value="<?php echo $bank->name ?>" required>
                                    </div>
                                    <div class="col-xl-6">
                                        <label class="form-label">Branch Name</label>
                                        <input type="text" class="form-control" id="branch"
                                            value="<?php echo $bank->branch ?>" required>
                                    </div>
                                    <div class="col-xl-6">
                                        <label class="form-label">Location</label>
                                        <input type="text" class="form-control" id="location"
                                            value="<?php echo $bank->location ?>">
                                    </div>
                                    <div class="col-xl-6">
                                        <label class="form-label">Phone Number</label>
                                        <input type="tel" class="form-control" id="phone"
                                            value="<?php echo $bank->phone ?>" required>
                                    </div>
                                </div>
                                <div class="d-flex justify-content-end mt-4">
                                    <button class="btn text-white" onclick="updateBank()" style="background:#6AA32D;">
    ...
    function updateBank() {
        ...
        if (!$('#bank-name').val() || !$('#branch').val() || !$('#phone').val()) {
            toastr.error('Please fill in all required fields', 'Error');
            return;
        }
        $.ajax({
            url: 'http://localhost/dfcs/ajax/bank-controller/update-bank.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
                    ...
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.location.href =
                                'http://localhost/dfcs/admin/banks/view-data';
                        }, 2000);
                    } else {
