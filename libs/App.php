   ...
   public function connect()
{
    ...
    try {
        // Include the charset=utf8mb4 in the DSN
        ...
        // Optional: Set the character set explicitly (if needed)
    ...
    // Insert without path and return the last inserted ID
    public function insertWithoutPathAndReturnId($query, $arr)
    {
        $insert_record = $this->link->prepare($query);
        $insert_record->execute($arr);
        return $this->link->lastInsertId(); // Return the last inserted ID
    }
    ...
    // login in a distributed manner    
  public function verify_user_login($query, $data) {
    $login_user = $this->link->query($query);
    $login_user->execute();
    ...
    public function login_admin($query, $data)
    {
        $login_user = $this->link->query($query);
        $login_user->execute();
        ...
        if ($login_user->rowCount() > 0) {
            $fetch = $login_user->fetch(PDO::FETCH_ASSOC);
            if (password_verify($data['password'], $fetch['Admin_password'])) {
                // Successfully logged in
                $_SESSION['AdminEmail'] = $fetch['Admin_email'];
                return "success";
            }
        }
        return "unsuccess";
    ...
    public function countItemsByDayInPastWeek($table)
    {
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $dayDate = date('Y-m-d', strtotime("-$i days", strtotime($currentDate)));

            // Query to retrieve the count of items for the specific day
            $query = "SELECT COUNT(*) FROM $table WHERE DATE(created_at) = :dayDate";
            $stmt = $this->link->prepare($query);
            $stmt->execute(['dayDate' => $dayDate]);
            $count = $stmt->fetchColumn();

            // Add the count to the dailyCounts array
            $dailyCounts[$dayDate] = $count;
        }
    ...
    public function getDaysAgo($created_at)
    {
        $current_time = time();
