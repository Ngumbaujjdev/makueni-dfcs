...
<body>
            ...
            <div class="container-fluid">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                    <div>
                        ...
                        <?php endif; ?>
                        <span class="fs-semibold text-muted pt-5">SACCO Staff Management Dashboard</span>
                    </div>
                </div>
                                ...
                                <div class="d-flex align-items-top justify-content-between">
                                    <div>
                                        <span class="avatar avatar-md avatar-rounded"
                                            ...
                                            <i style="color:#6AA32D" class="fa-solid fa-users-viewfinder fs-16"></i>
                                        </span>
                                    </div>
                                    <div class="flex-fill ms-3">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div>
                                                <p class="text-white mb-0">Total Farmers</p>
                                                <?php
                                                    $query = "SELECT COUNT(*) as count FROM farmers";
                                                    $result = $app->select_one($query);
                                                    $total_farmers = ($result) ? $result->count : 0;
                                                    ?>
                                ...
                                <div class="d-flex align-items-top justify-content-between">
                                    <div>
                                        <span class="avatar avatar-md avatar-rounded" style="background:#6AA32D;">
                                            <i class="fa-solid fa-user-check fs-16"></i>
                                        </span>
                                    </div>
                                    <div class="flex-fill ms-3">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div>
                                                <p class="text-muted mb-0" style="color:#6AA32D;">Verified Farmers</p>
                                                <?php
                                                  $query = "SELECT COUNT(*) as count FROM farmers WHERE is_verified = 1";
                                                  $result = $app->select_one($query);
                                                  $verified_farmers = ($result) ? $result->count : 0;
                                                  ?>
                                                <h4 class="fw-semibold mt-1" style="color:#6AA32D;">
                                                    <?php echo $verified_farmers ?>
                                                </h4>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="card custom-card overflow-hidden">
                            <div class="card-body">
                                <div class="d-flex align-items-top justify-content-between">
                                    <div>
                                        <span class="avatar avatar-md avatar-rounded" style="background:#6AA32D;">
                                            <i class="fa-solid fa-user-clock fs-16"></i>
                                        </span>
                                    </div>
                                    <div class="flex-fill ms-3">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div>
                                                <p class="text-muted mb-0" style="color:#6AA32D;">Pending Verification
                                                </p>
                                                <?php
                                                  $query = "SELECT COUNT(*) as count FROM farmers WHERE is_verified = 0";
                                                  $result = $app->select_one($query);
                                                  $pending_farmers = ($result) ? $result->count : 0;
                                                  ?>
                                                <h4 class="fw-semibold mt-1" style="color:#6AA32D;">
                                                    <?php echo $pending_farmers ?>
                                                </h4>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="card custom-card overflow-hidden">
                            <div class="card-body">
                                <div class="d-flex align-items-top justify-content-between">
                                    <div>
                                        <span class="avatar avatar-md avatar-rounded" style="background:#6AA32D;">
                                            <i class="fa-solid fa-user-plus fs-16"></i>
                                        </span>
                                    </div>
                                    <div class="flex-fill ms-3">
                                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                                            <div>
                                                <p class="text-muted mb-0" style="color:#6AA32D;">New This Month</p>
                                                <?php
                                                 $query = "SELECT COUNT(*) as count FROM farmers 
                                                          WHERE MONTH(created_at) = MONTH(CURRENT_DATE()) 
                                                          AND YEAR(created_at) = YEAR(CURRENT_DATE())";
                                                 $result = $app->select_one($query);
                                                 $new_farmers = ($result) ? $result->count : 0;
                                                 ?>
                                                <h4 class="fw-semibold mt-1" style="color:#6AA32D;">
                                                    <?php echo $new_farmers ?>
                                                </h4>
                                            </div>
                                        </div>
                                    </div>
    ...
    function displayFarmers() {
        let displayFarmers = "true";
        $.ajax({
            url: "http://localhost/dfcs/ajax/farmer-controller/display-farmers.php",
            type: 'POST',
            data: {
                displayFarmers: displayFarmers,
            },
            success: function(data, status) {
                $('#displayFarmers').html(data);
            },
        });
    }

    function viewFarmer(farmerId) {
        window.location.href = 'http://localhost/dfcs/sacco/farmers/view-details?id=' + farmerId;
    }

    function verifyFarmer(farmerId) {
        if (confirm("Are you sure you want to verify this farmer?")) {
            $.ajax({
                url: "http://localhost/dfcs/ajax/farmer-controller/verify-farmer.php",
                type: "POST",
                data: {
                    farmerId: farmerId
                },
                success: function(data, status) {
                    toastr.success('Farmer verified successfully', 'Success', {
